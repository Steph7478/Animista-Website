  ...
  return (
    <>
      <div className="overflow-hidden bg-white font-text shadow-lg shadow-gray-200">
            ...
            filteredMenuItems.map((animation) => {
              ...
              return (
                <div
                  ...
                  } group relative flex min-w-[110px] cursor-pointer select-none items-center justify-center overflow-hidden rounded-full text-xs font-bold uppercase text-white sm:h-[30px] sm:flex-shrink-0 sm:text-nowrap sm:px-4 md:h-[110px] md:flex-shrink md:text-wrap md:px-[10px]`}
                >
                  <span className="relative z-10 sm:flex-shrink-0 md:flex-shrink">
                    {animation.name}
                  </span>
                  {!isActive && (
                  ...
                  {favoriteCount > 0 && (
                    <span className="top-5 z-10 flex-shrink-0 text-sm text-white opacity-50 sm:static sm:pl-2 md:absolute md:pl-0">
                      {favoriteCount} ♥
                    </span>
                  )}
                </div>
        ...
        {filteredDetailItems && (
          <div
            className={`animate-fade-slide-down w-full overflow-auto p-3 text-center scrollbar-none sm:flex sm:flex-nowrap md:grid md:grid-cols-5`}
            key={pathSegments[2]}
            ...
            {filteredDetailItems.map((options, index) => {
              ...
              return (
                <div
                  ...
                  } animate-fade-slide-down group relative select-none text-[15px] transition-all duration-200 ease-in-out sm:flex-shrink-0 sm:rounded-full sm:px-5 sm:py-1 md:rounded-sm md:p-1`}
                >
                  {options}{" "}
                  {isoptionsFavorite && (
                    <span
      ...
      <div className="relative flex h-full w-full flex-shrink-0 items-center justify-center overflow-hidden">
        ...
        <button
          ...
          className={`justify-cente absolute left-0 top-0 z-20 m-5 flex h-[40px] w-[40px] items-center rounded-full bg-white transition-transform duration-150 ease-in-out sm:block md:hidden ${
            isOpen || showSettings
        ...
        <div
          className={`${!isOpen ? "md:translate-x-0" : "md:-translate-x-[105%]"} absolute z-20 transition-all delay-300 duration-700 ease-in-out sm:inset-0 sm:h-full sm:w-full md:left-0 md:top-0 md:h-[477px] md:w-[300px] ${!showSettings ? "sm:-translate-x-[105%]" : "sm:translate-x-0"}`}
        >
