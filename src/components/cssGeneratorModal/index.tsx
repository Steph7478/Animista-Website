...
export default function CssModal({ isOpen, setIsOpen }: CssModalProps) {
  ...
  const handleCopy = (divRef: React.RefObject<HTMLDivElement | null>) => {
    if (divRef.current) {
  ...
  return (
    <div
      className={`${isOpen === true ? "translate-x-0 delay-300" : "translate-x-[105%] delay-200"} absolute inset-0 z-20 mt-6 flex h-[95%] w-[95%] flex-col self-start justify-self-center rounded-lg bg-white p-12 transition-all duration-1000 ease-in-out`}
    >
      <div className="flex flex-row gap-x-5 self-start pb-8">
        <label
      ...
      <button
        ...
        className="mt-8 self-start bg-navbar px-4 py-1 text-sm font-bold text-white transition-colors duration-100 hover:bg-lightred"
      >
        COPY KEYFRAMES
      </button>
      ...
      <div className="absolute bottom-10 right-12 flex items-center justify-center gap-x-2 text-[15px]">
