...
export default function Modal({
  ...
  return (
    <>
      ...
      <div
        className={`fixed bottom-[85px] right-5 z-20 mb-2 flex h-[475px] w-[400px] flex-col items-center justify-start overflow-y-scroll rounded-lg bg-white p-5 shadow-lg transition-transform ${modal ? "scale-100" : "scale-0"} origin-bottom-right overflow-x-hidden pb-20 duration-500 ease-in-out`}
      >
      ...
      <div
        className={`z-50 flex h-[63px] w-[63px] cursor-pointer items-center justify-center justify-self-end rounded-full bg-red-500 brightness-125 ${modal ? "hover:scale-100" : "hover:scale-110"} fixed bottom-5 right-5 transition-transform duration-300 ease-in-out`}
        onClick={toggleModal}
      >
        {!modal && (
          <img
        ...
        {modal && (
          <FaChevronDown className="h-[18px] w-[18px] text-white transition-transform duration-150 ease-in-out" />
        )}
      </div>
    </>
  );
