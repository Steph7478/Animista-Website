...
export default function Options({ setShowSettings }: OptionsProps) {
  ...
  const renderCheckbox = useCallback(
    (
      ...
      <div
        className={`${id === "infinite" ? "flex justify-start sm:basis-[41%] md:basis-[44%]" : ""}`}
      >
  ...
  return (
    <form className="shadow-xs relative z-20 m-5 bg-white p-5 font-text text-navbar shadow-black">
      <button
        ...
        className="group absolute right-0 top-0 h-10 w-10 p-2 sm:block md:hidden"
      >
      ...
      <fieldset className="space-y-2">
        ...
        <div className="relative flex gap-x-2">
          <input
            id="delay"
            type="text"
            className={`h-[28px] w-full bg-bgcolor px-2 text-sm font-semibold text-navbar placeholder:font-semibold sm:w-full`}
