...
export default function Header() {
  ...
  return (
    <div className="relative flex w-full flex-wrap items-center justify-start overflow-hidden bg-navbar font-text text-xs uppercase text-white md:h-[50px]">
      ...
      <ul className="ml-4 flex h-full items-center font-bold sm:order-last sm:w-full md:-order-none md:w-auto">
      ...
      <div
        className={`ml-auto flex p-2 opacity-50 ${totalFavs(animation.favorites) === 0 ? "opacity-20" : "opacity-50"}`}
      >
      ...
      <div
        className={`flex gap-x-[2px] brightness-[85%] ${totalFavs(animation.favorites) === 0 ? "pointer-events-none opacity-50" : "pointer-events-auto"}`}
      >
        <button
          onClick={toggleFilter}
          className={`flex h-[30px] w-[30px] items-center justify-center rounded-sm hover:brightness-125 ${animation.isFiltered === true ? "bg-lightred" : "bg-icons"}`}
        >
